...
import env
from advisor import (DEFAULT_MODEL, GeminiAdvisor, RuleAdvisor, SafeAdvisor, UsageLimiter, cost_usd, DAY)
...
def reply(decisions, tokens=(500, 100), wrap=''):
    ...
    return dict(candidates=[dict(content=dict(parts=[dict(text=text)]))], usageMetadata=dict(promptTokenCount=tokens[0], candidatesTokenCount=tokens[1]))
...
class LimiterTests(unittest.TestCase):
    ...
    def test_cost_estimate(self):
        self.assertAlmostEqual(cost_usd(DEFAULT_MODEL, 1_000_000, 1_000_000), 0.375)
        self.assertAlmostEqual(cost_usd('modelo-desconocido', 0, 1_000_000), 0.30)  # unknown = most expensive known
...
class GeminiAdvisorTests(unittest.TestCase):
    def advisor(self, transport, **kw):
        return GeminiAdvisor(KEY, transport=transport, **kw)
    ...
    def test_request_shape_and_no_secrets_in_prompt(self):
        ...
        self.assertTrue('generativelanguage.googleapis.com' in seen['url'])
        self.assertIn(KEY, seen['url'])
        self.assertEqual(seen['body']['generationConfig']['temperature'], 0.0)
        prompt = seen['body']['contents'][0]['parts'][0]['text']
        self.assertNotIn(KEY, prompt + seen['body']['systemInstruction']['parts'][0]['text'])
...
class SafeAdvisorTests(unittest.TestCase):
    def run_safe(self, transport, candidates=None):
        events = []
        safe = SafeAdvisor(GeminiAdvisor(KEY, transport=transport))
    ...
    def test_ai_can_reject_and_reasons_are_recorded(self):
        ...
        self.assertEqual([(e['symbol'], e['approve'], e['source']) for e in events],
                         [('BTCUSDT', False, 'gemini'), ('ETHUSDT', True, 'gemini')])
    ...
    def test_every_failure_falls_back_to_plan_b(self):
        ...
        kept, events = self.run_safe(lambda *a: dict(candidates=[dict(content=dict(parts=[dict(text='no soy json')]))]))
    ...
    def test_cannot_add_or_alter_candidates_through_the_trader(self):
        ...
        safe = SafeAdvisor(GeminiAdvisor(KEY, transport=lambda *a: reply(
