...
again; when the program itself is restarted it resumes from the saved state, open positions included.
    ...
    py bot.py review    # weekly review now (AI proposals only if ANTHROPIC_API_KEY is in .env)
...
def build_ai(db_path, values):
    ...
    claude = ai.ClaudeAdvisor(key, values.get('ADVISOR_MODEL', ai.DEFAULT_MODEL), limiter=limiter)
    return ai.SafeAdvisor(claude), weekly.ClaudeProposer(claude)
    ...
    try:
        if args.command == 'init':
            ...
            notifier = alerts.Notifier(alerts.from_env(values), describe=describe,
                                       secrets=[values.get('ANTHROPIC_API_KEY')])
            print(f'Bot en marcha ({args.account}). Analista: {"IA (Claude) con plan B" if reviewer else "solo reglas"}. '
