...
class VersionTests(unittest.TestCase):
    ...
    def test_one_candidate_at_a_time_and_weekly_cooldown(self):
        first = self.v.propose({'tp_r': 2.2}, 'motivo', 'claude')
        with self.assertRaises(ValueError):
            self.v.propose({'trail_atr': 3.2}, 'otro', 'claude')       # one at a time
        self.v._set(first, status='rejected')
        with self.assertRaises(ValueError):
            self.v.propose({'trail_atr': 3.2}, 'otro', 'claude')       # cooldown: at most one per week
        self.clock.days(8)
        self.assertTrue(self.v.propose({'trail_atr': 3.2}, 'otro', 'claude'))
        with self.assertRaises(ValueError):
            self.v.propose({'trail_atr': 3.2}, 'x', 'claude')

    def test_same_value_and_missing_baseline(self):
        with self.assertRaises(ValueError):
            self.v.propose({'tp_r': Params().tp_r}, 'igual', 'claude')
        with tempfile.TemporaryDirectory() as d:
            empty = Versions(os.path.join(d, 'e.db'))
            try:
                with self.assertRaises(ValueError):
                    empty.propose({'tp_r': 2.5}, 'x', 'claude')
            finally:
                empty.close()

    def test_backtest_gate_passes_a_clearly_better_candidate(self):
        self.gate(good=True)
        vid = self.v.propose({'tp_r': 2.5}, 'mejor', 'claude')
    ...
    def test_backtest_gate_rejects_each_kind_of_weakness(self):
        ...
        for kw, failed in zip(cases, expected):
            self.gate(**kw)
            self.clock.days(8)
            vid = self.v.propose({'tp_r': 2.5}, 'x', 'claude')
            evidence = self.v.evaluate(vid, rows={})
...
class PromotionTests(unittest.TestCase):
    ...
    def candidate_in_paper(self, change, days=True):
        vid = self.v.propose(change, 'motivo', 'claude')
...
class ReviewTests(unittest.TestCase):
    ...
    def test_claude_proposer_parses_and_sends_no_secrets(self):
        sent = []
        ...
        stats = review.week_stats(self.paper, 'main', self.now)
        got = review.ClaudeProposer(Ask()).propose(stats, Params())
